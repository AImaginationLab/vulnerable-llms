...
const LLM08Page: React.FC = () => {
  ...
  const handleInvertVector = async () => {
    ...
    try {
      ...
      alert(
        'Error: Unable to invert embedding. The document may not have a valid ID.'
      );
    }
    setLoading(false);
  };
  ...
  const handleDemoEmbed = async () => {
    ...
    try {
      ...
      if (resp.data.embedding) {
        ...
        const simpleEmbedding = Array.from(
          { length: 12 },
          (_, i) =>
            Math.sin((demoText.charCodeAt(i % demoText.length) + i) * 0.1) *
            (Math.random() * 0.5 + 0.5)
        );
    ...
    >
      ...
      <div className="demo-section">
        ...
        <Card>
          <p style={{ marginBottom: '16px' }}>
            <strong>What are embeddings?</strong> When text is stored in a
            vector database, it's converted into a series of numbers (a vector)
            that represents its meaning. Think of it like a fingerprint for
            text.
          </p>
          ...
          {demoEmbedding.length > 0 && (
            <div className="output-panel">
              ...
              <div className="help-text">
                💡 Each number represents a different aspect of your text's
                meaning. Real embeddings have hundreds or thousands of
                dimensions!
              </div>
            </div>
          )}
      ...
      <div className="demo-section">
        ...
        <div className="attack-steps">
          ...
          <div
            ...
            onClick={() => {
              ...
              else if (currentStep < 1 && stolenVectors.length > 0)
                setCurrentStep(1);
            }}
            style={{
              cursor:
                currentStep > 1 || (currentStep < 1 && stolenVectors.length > 0)
                  ? 'pointer'
                  : 'default',
            }}
          >
          ...
          <div
            ...
            onClick={() =>
              currentStep > 2 && inversionResult && setCurrentStep(2)
            }
            style={{
              cursor:
                currentStep > 2 && inversionResult ? 'pointer' : 'default',
            }}
          >
        ...
        {currentStep === 0 && (
          <Card>
            <Alert type="warning" title="The Security Risk">
              If an attacker gains access to these embeddings, they might be
              able to reverse-engineer the original text using mathematical
              techniques. Let's see how this attack works...
            </Alert>
        ...
        {currentStep >= 1 && stolenVectors.length > 0 && (
          <Card
            title="📥 Step 2: Select Target Embedding"
            className="step-card"
          >
            {currentStep === 1 ? (
              <>
                <Alert type="danger" title="🔓 Unauthorized Access Simulated">
                  The attacker has gained access to the vector database and
                  retrieved {stolenVectors.length} embeddings.
                </Alert>
        ...
        {currentStep >= 2 && inversionResult && (
          <Card
            ...
            className="step-card"
          >
            {inversionResult.inversion_results &&
            inversionResult.inversion_results.length > 0 &&
            inversionResult.inversion_results[0].candidates &&
            inversionResult.inversion_results[0].candidates.length > 0 ? (
              <>
                <Alert type="danger" title="⚠️ Information Leaked!">
                  The inversion attack successfully recovered potential text
                  from the embedding:
                </Alert>
                ...
                {inversionResult.inversion_results[0].candidates
                  .filter(
                    (c: any) => c.method === 'llm_assisted_reconstruction'
                  )
                  .map((candidate: any) => (
                    <div
                      key="llm-reconstruction"
                      className="reconstruction-highlight"
                    >
                      ...
                      <p className="help-text">
                        This coherent sentence was reconstructed using AI from
                        the recovered word fragments
                        {candidate.metadata?.estimated_length && (
                          <span style={{ display: 'block', marginTop: '4px' }}>
                            📏 Estimated original length: ~
                            {candidate.metadata.estimated_length} words (based
                            on embedding entropy:{' '}
                            {candidate.metadata.embedding_entropy})
                          </span>
                        )}
                      </p>
                    </div>
                  ...
                  <Card title="🔬 How We Reconstructed the Text">
                    <div style={{ fontSize: '14px', lineHeight: '1.6' }}>
                      ...
                      <div style={{ marginBottom: '16px' }}>
                        ...
                        >
                          We analyzed the mathematical properties of the
                          embedding vector to understand its information
                          content:
                          {inversionResult.inversion_results[0].candidates[0]
                            ?.metadata?.embedding_entropy && (
                            <span
                              style={{ display: 'block', marginTop: '4px' }}
                            >
                              • Entropy:{' '}
                              {
                                inversionResult.inversion_results[0]
                                  .candidates[0].metadata.embedding_entropy
                              }
                              (higher = more complex information)
                            </span>
                          )}
                          {inversionResult.inversion_results[0].candidates[0]
                            ?.metadata?.active_dimensions && (
                            <span style={{ display: 'block' }}>
                              • Active dimensions:{' '}
                              {
                                inversionResult.inversion_results[0]
                                  .candidates[0].metadata.active_dimensions
                              }
                              (more = richer content)
                            </span>
                          )}
                      ...
                      <div style={{ marginBottom: '16px' }}>
                        ...
                        >
                          We compared the embedding against a database of known
                          word embeddings using cosine similarity. Words with
                          high similarity scores are likely components of the
                          original text.
                          {inversionResult.inversion_results[0].candidates[0]
                            ?.metadata?.high_confidence_words?.length > 0 && (
                            <span
                              style={{ display: 'block', marginTop: '4px' }}
                            >
                              • High confidence words:{' '}
                              {inversionResult.inversion_results[0].candidates[0].metadata.high_confidence_words.join(
                                ', '
                              )}
                            </span>
                          )}
                      ...
                      <div style={{ marginBottom: '16px' }}>
                        ...
                        >
                          We identified word pairs that commonly appear together
                          in text to improve reconstruction accuracy.
                          {inversionResult.inversion_results[0].candidates[0]
                            ?.metadata?.word_pairs?.length > 0 && (
                            <span
                              style={{ display: 'block', marginTop: '4px' }}
                            >
                              • Detected pairs:{' '}
                              {inversionResult.inversion_results[0].candidates[0].metadata.word_pairs.join(
                                ', '
                              )}
                            </span>
                          )}
                      ...
                      <div style={{ marginBottom: '16px' }}>
                        ...
                        >
                          Using information theory, we estimated the original
                          text was approximately{' '}
                          {inversionResult.inversion_results[0].candidates[0]
                            ?.metadata?.estimated_length || 'unknown'}{' '}
                          words long based on the embedding's information
                          density.
                        </p>
                      </div>
                      ...
                      <div>
                        ...
                        >
                          Finally, we used an AI model to create a coherent
                          sentence from the recovered words, respecting the
                          estimated length and prioritizing high-confidence
                          words.
                        </p>
                      </div>
                    </div>
              ...
              <Alert type="success" title="✅ Inversion Failed">
                The embedding could not be inverted to recover meaningful
                information.
              </Alert>
            )}
          ...
          >
            <Card>
              ...
              <p style={{ fontSize: '14px', margin: 0 }}>
                Many companies use vector databases for semantic search, RAG
                systems, and recommendation engines. Without proper security,
                sensitive customer data, proprietary information, or personal
            ...
            <Card>
              ...
              <p style={{ fontSize: '14px', margin: 0 }}>
                Attackers might gain access through database vulnerabilities,
                insider threats, misconfigured cloud storage, or supply chain
                attacks on vector database providers.
              </p>
            </Card>
            ...
            <Card>
              ...
              <p style={{ fontSize: '14px', margin: 0 }}>
                Implement defense-in-depth: encryption, access controls,
                monitoring, differential privacy, and regular security audits of
                your vector infrastructure.
              </p>
            </Card>
          </div>
        </div>
